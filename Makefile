...
build:
	...
	go build -o family -ldflags="\
    	-X 'github.com/FamilyChain/family/versioning.Version=$(LATEST_VERSION)' \
		-X 'github.com/FamilyChain/family/versioning.Commit=$(COMMIT_HASH)'\
		-X 'github.com/FamilyChain/family/versioning.Branch=$(BRANCH)'\
		-X 'github.com/FamilyChain/family/versioning.BuildTime=$(TIME)'" \
	main.go
...
test-e2e:
	...
	go build -race -o artifacts/family .
	env EDGE_BINARY=${PWD}/artifacts/family go test -v -timeout=30m ./e2e/...

.PHONY: test-e2e-polybft
test-e2e-polybft:
    # We can not build with race because of a bug in boltdb dependency
	go build -o artifacts/family .
	env EDGE_BINARY=${PWD}/artifacts/family E2E_TESTS=true E2E_LOGS=true E2E_TESTS_TYPE=integration \
	go test -v -timeout=45m ./e2e-polybft/e2e/...

test-property-polybft:
    # We can not build with race because of a bug in boltdb dependency
	go build -o artifacts/family .
	env EDGE_BINARY=${PWD}/artifacts/family E2E_TESTS=true E2E_LOGS=true E2E_TESTS_TYPE=property go test -v -timeout=30m ./e2e-polybft/property/...
